...
The predicted rewards outputted by the dynamics function is matched against the actual rewards received during the game of self-play. The predicted policy outputted by the prediction function is matched against the policy outputted by the MCTS search. 

The "ground truth" for the value, is calculated using n-step bootstrapping, where n is specified in the config parameter. If n is a number equal to or larger than the episode length, then the value is calculated using the actual discounted transition rewards of the game of self-play, and reduces to the Monte Carlo return. If n is less than the episode length, then discounted transition rewards are used until the n-step, at which the value outputted by the MCTS search is used to bootstrap. The predicted value outputted by the prediction function is then matched against this calculated value.
