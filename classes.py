...
class Game: # wrapper for gym env
    ...
    def apply_action(self,action_index): # this method should never be called when self.at_terminal_state is True
...
class Node: # MCTS node
    ...
    def get_ucb_score(self,visit_sum,min_q_value,max_q_value,config): # visit_sum is the sum of all visits across all children nodes (for this node's parent)
        ...
        if min_q_value != max_q_value: normalized_q_value = (normalized_q_value - min_q_value) / (max_q_value - min_q_value) # normalize q-value
